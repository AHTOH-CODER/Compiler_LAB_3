...
class Parser:
    ...
    def parse(self) -> ParseResult:
        ...
        while self._current() is not None:
            if self.pos > 0 and self.tokens[self.pos - 1].value == ';' and (self._current().token_type == 'ERROR'):
    ...
    def _parse_complex_declaration(self):
        ...
        if not self._expect_identifier():
            ...
            if self._check_value(';') or self._current() is None:
                ...
                return
        if not self._expect_value('=', "Ожидался оператор '='", "Expected '=' operator"):
        ...
        if not self._is_keyword('Complex'):
            self._report_current_error(self._m("Ожидалось ключевое слово 'Complex'", "Expected keyword 'Complex'"))
